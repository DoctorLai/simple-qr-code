document.addEventListener(
  ...
  function () {
    chrome.tabs.query({ currentWindow: true, active: true }, (tabs) => {
      ...
      chrome.storage.session.get("pendingQrText", function (pending) {
        const pendingText =
          pending && typeof pending.pendingQrText === "string"
            ? pending.pendingQrText
            : "";
        const initialText = pendingText.length > 0 ? pendingText : url;
        if (pendingText.length > 0) {
          chrome.storage.session.remove("pendingQrText");
        }
        ...
        chrome.storage.sync.get(
          ...
          function (stored) {
            ...
            function updateQR(text, successMessage) {
              ...
              setStatus(successMessage || appName);
            }
